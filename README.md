...
После запуска API оркестратора: по умолчанию Makefile шлёт тот же секрет, что и compose (`dev-change-me`), если вы не задавали свой при `make orchestrator-up`.
...
Если ответ **401 invalid token**: смотрите `make orch-webhook-secret` и передайте тот же секрет в `make apply-plan-bundle WEBHOOK_SECRET='…'`. **Важно:** если в shell был `export WEBHOOK_SECRET=…`, при `docker compose` он перебивает `.env` — поэтому `make orchestrator-up` сбрасывает его на время вызова compose; после смены `.env` выполните `make orchestrator-up` ещё раз, затем `apply-plan-bundle` с тем же секретом.
...
Создайте `.env` в рабочей директории MCP-сервера или задайте переменные в настройках Cursor.
...
- Пример данных: `examples/planbundle_rn_auth.json`, сид: `scripts/seed_rn_auth.sql`.
